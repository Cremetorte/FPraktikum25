...
file = files[0]  # Change to the desired file
...
peak_1_prime_bounds = [0.0232, 0.025]
...
peaks_low_bounds = [0.005, 0.01, 0.0145, 0.019]
peaks_high_bounds = [0.0060, 0.011, 0.016, 0.02]
peaks = []
...
plt.plot(t*factor, channel_1, label='Channel 1', color='blue')
plt.vlines(peak_1*factor, ymin=min(channel_1), ymax=max(channel_1*1.2), color='red', linestyle='--', label='(0,0))')
plt.vlines(peak_1_prime*factor, ymin=min(channel_1), ymax=max(channel_1*1.2), color='red', linestyle='--')
plt.annotate("(0,0)", xy=(peak_1*factor, max(channel_1)), xytext=(3, 10), textcoords='offset points', 
                 arrowprops=dict(arrowstyle='-', color='red'), color='red', fontsize=10)
plt.annotate("(0,0)", xy=(peak_1_prime*factor, max(channel_1)), xytext=(3, 10), textcoords='offset points', 
                 arrowprops=dict(arrowstyle='-', color='red'), color='red', fontsize=10)
...
plt.title('Resonator spectrum for l=0') 
